...
import { Basket as BasketModel } from "../models/basket"; // Aliasing necessário para evitar conflito com a constante Basket
...
const Account = {
    login: (values: any) => requests.post('auth/login', values) // Mantido o seu uso do any
};

const Orders = {
    list: () => requests.get('orders'),
    fetch: (id: number) => requests.get(`orders/${id}`),
    create: (values: any) => requests.post('orders', values) // Mantido o seu uso do any
};

// Exportação Nomeada estável para o ecossistema do Vite
export const agent = {
    Store,
    Basket,
    Account,
